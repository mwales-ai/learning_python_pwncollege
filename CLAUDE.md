...
* `getting-started` - `hello-hacker`, `birthday-banner`
* `loops-and-lists` - `secret-decoder-ring`

Note the gap: these are README modules 1 and 4.  Modules 2 (Talking to the
Terminal) and 3 (Making Decisions) are not built yet, so `dojo.yml` currently
lists a module 1 followed by a module 4.  Insert the missing modules into
`dojo.yml` in README order when they get written.
...
* Only 3 of 34 planned challenges exist.
