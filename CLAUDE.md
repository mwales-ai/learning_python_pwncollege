...
Note the gaps: these are README modules 1, 4 and 5.  Modules 2 (Talking to the
Terminal) and 3 (Making Decisions) are not built yet, so `dojo.yml` currently
jumps from module 1 to module 4.  Insert the missing modules into `dojo.yml`
in README order when they get written.
...
* Only 4 of 35 planned challenges exist.
